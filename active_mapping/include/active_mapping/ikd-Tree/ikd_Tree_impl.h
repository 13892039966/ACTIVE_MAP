...
void KD_TREE<PointType>::Build(PointVector point_cloud){
    ...
    InitTreeNode(STATIC_ROOT_NODE); // wgh 内存依赖上的根节点。
        ...
        if (downsample_switch){
            ...
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){  
                if (Downsample_Storage.size() > 1 || same_point(PointToAdd[i], downsample_result)){
                    ...
                    const int root_axis = (Root_Node != nullptr) ? Root_Node->division_axis : 0;
                    Add_by_point(&Root_Node, downsample_result, true, root_axis);
                    tmp_counter ++;                      
                }
            // wgh-- 如果有re-balancing任务在并行运行，在对当前tree执行`BoxDelete`和`插入一个点`之外，还需要把这些操作缓存到logger里。
                ...
                if (Downsample_Storage.size() > 1 || same_point(PointToAdd[i], downsample_result)){
                    ...
                    const int root_axis = (Root_Node != nullptr) ? Root_Node->division_axis : 0;
                    Add_by_point(&Root_Node, downsample_result, false, root_axis);
                    tmp_counter ++;
                    if (rebuild_flag){
                        pthread_mutex_lock(&rebuild_logger_mutex_lock);
            ...
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
                const int root_axis = (Root_Node != nullptr) ? Root_Node->division_axis : 0;
                Add_by_point(&Root_Node, PointToAdd[i], true, root_axis);
            } 
            // wgh 如果有并行re-balancing任务，还需额外把当前操作放入logger缓存。
                ...
                pthread_mutex_lock(&working_flag_mutex);
                const int root_axis = (Root_Node != nullptr) ? Root_Node->division_axis : 0;
                Add_by_point(&Root_Node, PointToAdd[i], false, root_axis);
                if (rebuild_flag){
                    pthread_mutex_lock(&rebuild_logger_mutex_lock);
                    Rebuild_Logger.push(operation);
                    pthread_mutex_unlock(&rebuild_logger_mutex_lock);
...
void KD_TREE<PointType>::Rebuild(KD_TREE_NODE ** root){
    ...
    if ((*root)->TreeSize >= Multi_Thread_Rebuild_Point_Num) { 
        ...
        if (father_ptr == nullptr && *root == Root_Node) {
            father_ptr = STATIC_ROOT_NODE;
        }
        ...
        if (*root == Root_Node && STATIC_ROOT_NODE != nullptr) STATIC_ROOT_NODE->left_son_ptr = *root;
    } 
    return;
}
...
{     
    ...
    if (*root == nullptr){
        *root = new KD_TREE_NODE;
        ...
        if (root == &Root_Node) {
            (*root)->father_ptr = STATIC_ROOT_NODE;
            if (STATIC_ROOT_NODE != nullptr) {
