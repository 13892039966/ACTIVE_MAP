...
/*-----This part below is added by myself, which function is used to publish the global map in the formation of pointcloud---------*/
        ...
        objects_list = airsim_client_->simListSceneObjects(global_map_object_filter_);
        ROS_INFO_STREAM("Global map object filter: " << global_map_object_filter_
                        << ", matched " << objects_list.size() << " scene objects");
        ...
        {
            ROS_WARN_STREAM("No scene objects matched filter '" << global_map_object_filter_
                            << "'. Publishing an empty /airsim_global_map.");
        }
        ...
        ROS_INFO_STREAM("Published /airsim_global_map with " << cloudMap.points.size() << " points");
...
void AirsimROSWrapper::initialize_ros()
{
    ...
    nh_private_.param("global_map_object_filter", global_map_object_filter_, global_map_object_filter_);
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles)
    {
        ...
        airsim_map_pub = nh_private_.advertise<sensor_msgs::PointCloud2>("/airsim_global_map", 1, true);
...
sensor_msgs::Imu AirsimROSWrapper::get_imu_msg_from_airsim(const msr::airlib::ImuBase::Output& imu_data) const
{
    ...
    if (isENU_)
    {
        ...
        Eigen::Matrix3d rot_ned_frd = attitude_quat.toRotationMatrix();
        Eigen::Matrix3d ned_to_enu;
        ned_to_enu << 0, 1, 0,
                      1, 0, 0,
                      0, 0, -1;
        Eigen::Matrix3d frd_to_flu;
        frd_to_flu << 1, 0, 0,
                      0, -1, 0,
                      0, 0, -1;

        Eigen::Matrix3d rot_enu_flu = ned_to_enu * rot_ned_frd * frd_to_flu;
        Eigen::Quaterniond enu_quat(rot_enu_flu);

        imu_msg.orientation.w = enu_quat.w();
        imu_msg.orientation.x = enu_quat.x();
        imu_msg.orientation.y = enu_quat.y();
        imu_msg.orientation.z = enu_quat.z();
