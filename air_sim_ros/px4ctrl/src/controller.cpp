...
)
{
	...
	*/
	...
	const double limit_rate = 1.5 * M_PI;
	if (std::fabs(u.roll_rate) >= limit_rate) {
		ROS_WARN_THROTTLE(
			0.5, "ROLL RATE LIMITED: cmd=%.3f rad/s limit=%.3f rad/s",
			u.roll_rate, limit_rate);
	}
	if (std::fabs(u.pitch_rate) >= limit_rate) {
		ROS_WARN_THROTTLE(
			0.5, "PITCH RATE LIMITED: cmd=%.3f rad/s limit=%.3f rad/s",
			u.pitch_rate, limit_rate);
	}
