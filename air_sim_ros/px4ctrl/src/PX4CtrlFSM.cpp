...
void PX4CtrlFSM::process_cmd_control(Controller_Output_t& u, SO3_Controller_Output_t& u_so3)
{
	...
	const Eigen::Vector3d pos_err = des.p - odom_data.p;
	const Eigen::Vector3d vel_err = des.v - odom_data.v;
	const double odom_yaw = get_yaw_from_quaternion(odom_data.q);
	double yaw_err = des.yaw - odom_yaw;
	while (yaw_err > M_PI) yaw_err -= 2.0 * M_PI;
	while (yaw_err < -M_PI) yaw_err += 2.0 * M_PI;

	ROS_WARN_THROTTLE(
		0.5,
		"[px4ctrl dbg] odom_p=(%.2f %.2f %.2f) cmd_p=(%.2f %.2f %.2f) pos_err=(%.2f %.2f %.2f) "
		"odom_v=(%.2f %.2f %.2f) cmd_v=(%.2f %.2f %.2f) vel_err=(%.2f %.2f %.2f) "
		"odom_yaw=%.2f cmd_yaw=%.2f yaw_err=%.2f",
