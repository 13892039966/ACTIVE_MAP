...
void MapROS::cloudPoseCallback(const sensor_msgs::PointCloud2ConstPtr& msg,
  ...
  geometry_msgs::TransformStamped cloud_tf;
  try {
    cloud_tf = tf_buffer_->lookupTransform(frame_id_, msg->header.frame_id, msg->header.stamp,
                                           ros::Duration(0.2));
  } catch (const tf2::TransformException& ex) {
    ROS_WARN_THROTTLE(2.0, "cloudPoseCallback TF lookup failed: target=%s source=%s reason=%s",
                      frame_id_.c_str(), msg->header.frame_id.c_str(), ex.what());
    return;
  }

  camera_pos_(0) = cloud_tf.transform.translation.x;
  camera_pos_(1) = cloud_tf.transform.translation.y;
  camera_pos_(2) = cloud_tf.transform.translation.z;
  if (!map_->isInMap(camera_pos_)) return;

  camera_q_ = Eigen::Quaterniond(cloud_tf.transform.rotation.w, cloud_tf.transform.rotation.x,
                                 cloud_tf.transform.rotation.y, cloud_tf.transform.rotation.z);

  pcl::PointCloud<pcl::PointXYZ> cloud_world;
  cloud_world.points.reserve(cloud_in.points.size());
  const Eigen::Matrix3d camera_r = camera_q_.toRotationMatrix();

  for (const auto& pt_in : cloud_in.points) {
    if (!std::isfinite(pt_in.x) || !std::isfinite(pt_in.y) || !std::isfinite(pt_in.z)) continue;

    const Eigen::Vector3d pt_sensor(pt_in.x, pt_in.y, pt_in.z);
    const Eigen::Vector3d pt_world = camera_r * pt_sensor + camera_pos_;
    pcl::PointXYZ pt_out;
    pt_out.x = pt_world.x();
    pt_out.y = pt_world.y();
    pt_out.z = pt_world.z();
    cloud_world.points.push_back(pt_out);
  }
  ...
  map_->inputPointCloud(cloud_world, static_cast<int>(cloud_world.points.size()), camera_pos_);
