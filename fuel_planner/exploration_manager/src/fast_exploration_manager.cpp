...
bool findNearestNavigableFreePoint(
    const shared_ptr<SDFMap>& sdf_map, const Vector3d& start_pos, Vector3d& free_pos) {
  if (!sdf_map || !sdf_map->isInMap(start_pos)) return false;

  Eigen::Vector3i start_idx;
  sdf_map->posToIndex(start_pos, start_idx);

  const double resolution = sdf_map->getResolution();
    ...
    for (int dx = -radius; dx <= radius; ++dx) {
      for (int dy = -radius; dy <= radius; ++dy) {
        for (int dz = -radius; dz <= radius; ++dz) {
          ...
          if (!sdf_map->isInMap(idx)) continue;
          if (sdf_map->getOccupancy(idx) != SDFMap::FREE) continue;
          if (sdf_map->getInflateOccupancy(idx) != 0) continue;

          Vector3d candidate;
          sdf_map->indexToPos(idx, candidate);
          const double dist2 = (candidate - start_pos).squaredNorm();
          if (dist2 < best_dist2) {
            best_dist2 = dist2;
            free_pos = candidate;
            found = true;
...
int FastExplorationManager::planExploreMotion(
  ...
  const bool start_in_map = sdf_map_->isInMap(pos);
  const int start_occ = start_in_map ? sdf_map_->getOccupancy(pos) : -1;
  const int start_inflate = start_in_map ? sdf_map_->getInflateOccupancy(pos) : -1;
  if (start_in_map && start_occ == SDFMap::UNKNOWN) {
    Vector3d snapped_start;
    if (findNearestNavigableFreePoint(sdf_map_, pos, snapped_start)) {
      search_start = snapped_start;
      ROS_WARN_STREAM("Exploration start snapped from unknown cell " << pos.transpose() << " to "
                                                                     << search_start.transpose());
    }
  }

  planner_manager_->path_finder_->reset();
  if (planner_manager_->path_finder_->search(search_start, next_pos) != Astar::REACH_END) {
    const bool goal_in_map = sdf_map_->isInMap(next_pos);
    Eigen::Vector3i start_idx(-1, -1, -1);
    Eigen::Vector3i goal_idx(-1, -1, -1);
    if (start_in_map) sdf_map_->posToIndex(search_start, start_idx);
    if (goal_in_map) sdf_map_->posToIndex(next_pos, goal_idx);

    const int goal_occ = goal_in_map ? sdf_map_->getOccupancy(next_pos) : -1;
    const int goal_inflate = goal_in_map ? sdf_map_->getInflateOccupancy(next_pos) : -1;
    ...
    ROS_ERROR_STREAM(
        ...
        << " raw_start=" << pos.transpose()
        << " raw_occ=" << occToString(start_occ)
        << " raw_inflate_occ=" << start_inflate
